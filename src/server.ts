...
app.get("/encuestas/graficas", async (req: RequestWithUsuario, res: Response) => {
    try {
        const reprobadosPorUnidad = await encuestaRepository.numeroReprobados(myDataSource)
        const reprobadosPorProfesor = await encuestaRepository.numeroReprobadosPorProfesor(myDataSource)
        const problematicasPorEncuesta = await encuestaRepository.obtenerProblematicasPorEncuesta(myDataSource)
        const dificultadesPorGrupo = await encuestaRepository.obtenerUltimasDificultades(myDataSource)
        res.json({
            reprobadosPorUnidad,
            reprobadosPorProfesor,
            problematicasPorEncuesta,
            dificultadesPorGrupo
        })
    } catch (error) {
        console.error(error)
        res.status(500).json({ message: "Error al obtener las encuestas" })
...
app.put("/encuestas/:id", async (req: RequestWithUsuario, res: Response) => {
    ...
    const mayorDificultad = req.body.mayorDificultad
    try {
        const nuevaEncuesta = await encuestaRepository.modificarEncuesta(myDataSource, Number(id), encuesta)
        const encuestaProblematicas: EncuestaProblematicasGrupo = {
            id: undefined,
            encuestaId: nuevaEncuesta.id,
            problematicasGrupoId: problematica,
        ...
        await encuestaProblematicasGrupoRepository.eliminarEncuestaProblematicaGrupo(myDataSource, Number(id))
        await dificultadEstudianteRepository.eliminarDificultadEstudiante(myDataSource, Number(id))
        await encuestaProblematicasGrupoRepository.crearEncuestaProblematicasGrupo(myDataSource, encuestaProblematicas)
        mayorDificultad.forEach(async (item: any) => {
            let dificultadEstudiante: DificultadEstudiantes = {
                id: undefined,
